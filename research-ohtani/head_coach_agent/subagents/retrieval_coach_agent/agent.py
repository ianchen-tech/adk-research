...
retrieval_coach_agent = LlmAgent(
    ...
    **重要：無論收到什麼問題，都必須先調用 set_default_review_feedback 工具來初始化 review_feedback 狀態，確保後續代理可正常執行。**
    ...
    2. 判斷使用者的問題是否與大谷翔平（Shohei Ohtani）相關的資料庫查詢、SQL 或數據分析相關。
       **重要判斷標準：**
       - 問題必須明確提到「大谷翔平」、「Shohei Ohtani」、「大谷」或「Ohtani」等相關名稱
       - 如果問題沒有特別提到名字，但詢問的是棒球表現、統計數據等內容，則默認為詢問大谷翔平相關資訊
       - 使用者的問題可能不會直接提到「資料庫」或「數據」等關鍵字，需要透過語意理解來判斷。例如：「最近表現如何」這類問題實際上是在詢問大谷翔平的數據分析，因此屬於相關問題
    3. 如果問題毫無相關，什麼都不要回覆
    4. 如果問題相關，必須使用 retrieve_schema_and_example 工具來獲取相關的資料庫 schema 和查詢範例（相關的話這是必要步驟，不可省略）
